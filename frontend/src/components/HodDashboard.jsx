...
import { 
  ...
  CheckCircle, Target, ChevronRight, Zap, 
  Calendar, Search, Bell, BarChart3, TrendingUp 
} from 'lucide-react';
...
export const HodDashboard = () => {
  ...
  useEffect(() => {
    const fetchHodStats = async () => {
      try {
        const response = await api.get('/results/hod-dashboard');
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4 flex-1">
            <div className="relative w-full max-w-md hidden md:block">
              ...
              <input 
                type="text" 
                placeholder="Search faculty or reports..." 
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="flex items-center gap-3 pl-2">
               <div className="text-right hidden sm:block">
                  <p className="text-xs font-black text-slate-900 leading-none">HOD {user?.branch}</p>
                  <p className="text-[10px] font-bold text-indigo-600 uppercase tracking-tight mt-1">Department Admin</p>
               </div>
               <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-white font-black shadow-lg shadow-indigo-200">
                  {user?.branch?.charAt(0)}
               </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <motion.div 
            ...
            className="absolute -bottom-24 -left-24 w-96 h-96 bg-indigo-600 rounded-full blur-[100px]"
          />
          ...
          <div className="relative z-10 grid lg:grid-cols-12 gap-12 items-center">
            <div className="lg:col-span-7 space-y-6">
              <div className="flex flex-wrap gap-3">
                <span className="px-4 py-1.5 rounded-full bg-blue-500/20 text-blue-300 text-[10px] font-black uppercase tracking-widest border border-blue-500/30 backdrop-blur-md flex items-center gap-2">
                  <Zap size={12} fill="currentColor" /> Department Live
                </span>
              ...
              <p className="text-slate-400 text-lg font-medium max-w-md">
                Overseeing <span className="text-white font-bold">{stats?.totalStudents || 0}</span> students and <span className="text-white font-bold">{stats?.totalTeachers || 0}</span> faculty members.
              </p>
              
              <div className="flex flex-wrap gap-4 pt-4">
                <button className="group bg-white text-slate-900 px-8 py-4 rounded-2xl font-bold hover:bg-blue-50 transition-all flex items-center gap-2 shadow-xl hover:scale-[1.02] active:scale-95">
                  Generate Report <BarChart3 size={18} className="group-hover:rotate-12 transition-transform"/>
                </button>
            ...
            {/* Circular Pass Rate Gauge */}
            <div className="lg:col-span-5 flex justify-center lg:justify-end">
              <div className="relative w-56 h-56 md:w-64 md:h-64 group">
                <div className="absolute inset-0 bg-blue-500/20 rounded-full blur-3xl group-hover:bg-blue-500/40 transition-colors" />
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-20">
                  ...
                  <span className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] mt-2">Avg. Pass Rate</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mb-14">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-2xl font-black text-slate-800 tracking-tight">Branch Metrics</h2>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <StatCard label="Total Students" value={stats?.totalStudents} icon={GraduationCap} color="blue" />
            <StatCard label="Total Teachers" value={stats?.totalTeachers} icon={Users} color="indigo" />
            <StatCard label="Tests Published" value={stats?.totalTests} icon={BookOpen} color="emerald" />
          </div>
        </div>
        ...
        <div className="mb-14">
           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <StatCard label="Total Attempts" value={stats?.totalAttempts} icon={Activity} color="purple" />
            <StatCard label="Avg. Passing Rate" value={`${stats?.overallPassPercentage}%`} icon={CheckCircle} color="emerald" />
            <StatCard label="Avg. Department Score" value={stats?.overallAverageScore} icon={Target} color="amber" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <ActionCard 
            ...
            desc="Review teacher activities, assigned courses, and test performance."
            color="indigo"
          />
          <ActionCard 
            icon={TrendingUp}
            title="Performance Trends"
            desc="Deep dive into year-over-year passing rates and score distributions."
            color="emerald"
            badge="Updated"
          />
          <ActionCard 
            ...
            desc="Verify test content and approve examination materials."
            color="purple"
          />
        </div>
      </div>
